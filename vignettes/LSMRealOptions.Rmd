...
The purpose of this vignette is to present worked examples of the functions available in the 'LSMRealOptions' package, with an emphasis on valuing capital investment projects through ROA, calculating critical values of investment, considering operational flexibility in investment projects and allowing for different stochastic assumptions in underlying assets. The remainder of this vignette is structured as follows. Section 2 presents the LSM Simulation method. Section 3 describes an application of the 'LSM.AmericanOption' function to value vanilla American put options. Section 4 presents an application of the 'LSM.RealOption' function to perform ROA on an arbitrary investment project example, with an algorithm to calculate critical values of investment presented. Section 5 extends this investment project example to allow for temporary suspension and permanent abandonment in the investment project, with a brief discussion on how this has influenced investment decisions. Section 6 presents several examples of valuing investment projects with differing assumptions in the processes and number of stochastic underlying variables.
...
This example has considered only 10,000 simulated price paths, however more price paths to ensure the american-style option value converges is recommended. The computational time of the LSM simulation method increases as the number of simulated price paths are considered. The convergance of the calculated value can be evaluated by considering the standard error of the option value.
...
Step 1 - Asset prices are simulated and placed into an array:
```{r}
AssetPrices <- array(dim = c(51, 1e3, 2))
for(i in seq_len(2)) AssetPrices[,,i] <- GBM.Simulate(n = 1e3, t = 1, mu = 0.06, sigma = c(0.2, 0.3)[i], S0 = c(38, 35)[i], dt = 1/50)
```

Step 2 - Simulated asset prices are provided as the underlying stochastically evolving state variables, and the maximum of the two asset prices are provided as the payoff at time $t$ for price path $i$:
```{r}
...
Simulated.values <- GBM.Simulate(n = N.simulations, t = TTM[i], mu = rf, sigma = sigma[i], S0 = S0[i], dt = dt)
...
As stated by Longstaff and Schwartz (2001), the first three Laguerre polynomials (i.e. degree = 3) is sufficient to obtain effective convegence of the algorithm in this scenario. Approximation of American option values are dependent on the number of basis functions used, the number of simulations and the size of the discrete time step of observations.
...
RevenuePrices <- GBM.Simulate(n = 1e3, t = Project.lifetime, mu = 0.06, sigma = 0.2, S0 = Initial.price, dt = dt)
...
## Apply the LSM simulation method to value the investment project:
...
matplot(forecasting.periods, cbind(ProjectValue$`Cumulative Investment Prob`, ProjectValue.OF$`Cumulative Investment Prob`), type = 'l', ylim = c(0,1), xlab = "Forecasting Horizon", ylab = "Cumulative Investment Proportion", main = "Cumulative Investment Proportion over Forecasting Horizon"); legend("right", c("ROV", "ROV + OF"),cex=0.8, col = 1:2, fill = 1:2)

```

The proportion of invested paths is higher when considering operational flexibilities due to the addition to the project value resulting from the options to suspend and abandon operations.
...
States.list <- apply(matrix(colnames(ProjectValue.OF$`Project States`)), 1, FUN = function(x) cbind.data.frame(x, ProjectValue.OF$`Project States`[,x], forecasting.periods))
...
repeat{
  ...
  if(it > 0){
    ...
    ## The WOV does not have to be exactly zero. Having it within a tolerance value can be adequate and decrease processing time:
    ...
    ## If the underlying asset impacts costs, and the iteration has pushed the price of the asset below zero, 
    ## it's never optimal to invest immediately:
...
print(c(NPV = NPV.crit.value, ROV = ROV.crit.value))
